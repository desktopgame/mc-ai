...
public final class ActionProtocol {
    ...
    public ActionProtocol(JsonObject o) {
        ...
        if (!Arrays.asList("goal_follow", "goal_stop", "goal_look", "goal_pickup", "goal_deposit", "owner_near",
                "low_health", "owner_out_of_range", "no_item_in_range", "inventory_empty",
                "unavailable_action").contains(string(o, "reasonCode"))) {
            throw new IllegalArgumentException("Invalid reason");
        }
        JsonObject d = o.getAsJsonObject("decision"); type = string(d, "action");
        if (type.equals("stop") || type.equals("pickup") || type.equals("deposit")) { keys(d, "action"); }
    ...
    public boolean safe(String goal, String companion, int dim, double health, double distanceSquared) {
        ...
        if (type.equals("deposit")) { return "deposit_items".equals(goal) && distanceSquared >= 0 && distanceSquared <= 1024; }
