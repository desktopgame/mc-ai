...
前回の2件は今回の差分で解消したと判断する。
`consumeSkill` の入口で要求開始からの経過時間を確認するため、期限切れ応答は初回claimにもlease更新にも使われない。
CompletionはRequestごとのvolatile欄へ移され、旧workerが新requestの完了通知を上書きする経路がなくなった。
未完了requestの6秒期限とcancelの最大3回試行も確認した。

Javaテストを `forge.ps1 test --rerun-tasks` で再実行し、65件成功（failures/errors=0）。今回の差分に新たな修正必須の問題は見つからなかった。
実ゲームでのpause/通信遅延試験は未実施。Pythonは変更がないため再実行していない。
以前から対象外の数量未確定・取消時の表示、再起動スクリプトの停止範囲は、この2件の解消とは別に残る。
コード・稼働プロセス・配置jarは変更していない。以下は修正前の指摘記録。
