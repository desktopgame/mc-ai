...
2026-09-26、ed1ca65時点のREADME/HANDOFFを保存。以下の「現在」「未実装」、PID、配置済みjar、テスト件数は当時の記述であり、現行状態を保証しない。現在の入口は [README](README.md)、残課題は [既知の問題](knwon_issue.md)。履歴資料内の古い段階の主張は修正せず保存している。
