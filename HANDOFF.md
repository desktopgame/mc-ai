...
自動テストはPython 146件・Java 82件。基本の収集・mine・collect_block（実ゲーム）を確認済み。Skill終端Social通知は自動テスト範囲（実ゲーム未確認）。硬化（P1〜5）は自動テスト範囲で、実機の危険条件は未検証。
...
| Minecraft | 終了状態 |
| 自動テスト | Python **146件**・Java **82件**・Forgeビルド成功 |
